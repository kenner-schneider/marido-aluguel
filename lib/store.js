...
/**
 * Apaga o chamado de vez. As fotos no Storage precisam ser removidas ANTES,
 * por quem chama — aqui só some o registro. No Postgres o "on delete cascade"
 * leva junto as linhas de chamado_fotos.
 */
async function excluirChamado(id) {
  if (usandoSupabase) {
    await sb(`chamados?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE' });
    return true;
  }
  const db = carregar();
  const antes = db.chamados.length;
  db.chamados = db.chamados.filter(c => c.id !== id);
  db.chamado_fotos = db.chamado_fotos.filter(f => f.chamado_id !== id);
  salvar(db);
  return db.chamados.length < antes;
}
